...
const SanPhamModal = ({ isOpen, onClose, data, fetchData }) => {
  const [formData, setFormData] = useState({
    ...data,
    image: null,
    price: data?.price ? data?.price : 'Liên hệ'
  });
  ...
  const onUpdate = async () => {
    ...
    let res
    if (data) {
      res = await SERVER.API.put(`/api/san-pham`, formToSend);
    } else {
      res = await SERVER.API.post(`/api/san-pham`, formToSend);
    }
