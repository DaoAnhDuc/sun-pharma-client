...
const SanPham = () => {
  ...
  const mapData = chunkArray(data.products, ITEM_IN_SLIDE);
  ...
  return (
    <div className="lg:py-30 py-16" style={{ backgroundImage: 'url("/images/background.png")' }}>
                ...
                {item.map((subItem, index) => (
                  <div className={`bg-white  rounded`} style={{ boxShadow: "0 1px 2px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.16)" }}>
                    <div className="overflow-hidden rounded-lg p-4">
                      <img src={getLinkImage(subItem?.image)} alt={`Slide ${index + 1}`} className="rounded bg-white w-full h-72 object-contain select-none transition-transform duration-300 ease-in-out transform hover:scale-105" />
                    </div>
